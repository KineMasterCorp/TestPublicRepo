...
class DummyFeedDataFetcher {
    static func fetch(with request: FeedDataRequest, fetchNext: Bool = false) -> [FeedDataInfo] {
        DummyFeedDataSource.fetch(with: request, fetchNext: fetchNext)
    }
}

class DummyFeedDataSource {
    static private let FETCH_NUMS_AT_TIME = 15
    
    static private var sources = DummyFeedDataSource.allSources()
    static private var fetchNumHistory: [String:Int] = .init()
    
    static func fetch(with request: FeedDataRequest, fetchNext: Bool) -> [FeedDataInfo] {
        let from = fetchNext ? fetchNumHistory[request.target] ?? 0 : 0
        let limit = from + FETCH_NUMS_AT_TIME
        var filteredSources = [FeedDataInfo]()
        ...
        print("fetched from: \(from) until \(from + filteredSources.count), fetchNext: \(fetchNext)")
        
        return filteredSources
    }
        
    static func allSources() -> [FeedDataInfo] {
        return [
